# import necessary libraries including tensorflow
...
random.seed(0)
# The number of key features for heart failure dataset.
ks = {'heart': 12}
...
def L2X(datatype, train=True):
    ...
    activation = 'relu' if datatype in ['heart'] else 'selu'
    ...
    if train:
        ...
        model.compile(loss='categorical_crossentropy',
                      optimizer=adam,
                      metrics=['acc'])
        filepath = "./{}/L2X.hdf5".format(datatype)
        checkpoint = ModelCheckpoint(filepath, monitor='val_acc',
                                     verbose=1, save_best_only=True, mode='max')
        callbacks_list = [checkpoint]
        model.fit(x_train, y_train, validation_data=(x_val, y_val), callbacks=callbacks_list, epochs=3,
                  batch_size=BATCH_SIZE)
        st2 = time.time()
    else:
        model.load_weights('./{}/L2X.hdf5'.format(datatype),
                           by_name=True)
